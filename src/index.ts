...
function onResults(results: mpHands.Results): void {
  ...
  if (results.multiHandLandmarks && results.multiHandedness) {
    for (let index = 0; index < results.multiHandLandmarks.length; index++) {
      ...
      // if distance is in the hundredth, we can say that we have no distance between them
      console.log('distance', calculateDistanceBetweenLandMarks(landmarks[4], landmarks[8]));
